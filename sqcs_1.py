...
class CosineSimilarity:
    ...
    def compute_scores(self, img_batch, gen_img_batch):
        ...
        with torch.no_grad():
            ...
            emb_batch_gen = model(gen_img_batch).detach().cpu()
            mean_scores = []
            for i in range(emb_batch_real.shape[0]):
                rep_real = emb_batch_real[i,:].repeat(emb_batch_gen.shape[0],1) # Get the same size as the emb_batch_gen features
                scores = torch.nn.functional.cosine_similarity(rep_real.unsqueeze(1), emb_batch_gen.unsqueeze(0), dim=2) # Change the rep_real accordingly
                mean_scores.append(torch.mean(scores))
